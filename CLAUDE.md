...
- **`signal_daily`** — Derived table, rebuilt by `make signals` (TRUNCATE + rebuild, like `port_events`) from the classified voyage legs (`pipeline/legs.py`) + `port_events`. Tidy/long daily panel: one row per `(signal_key, bucket_date, zone_scope, regime, basis)`. First-build `signal_key`s: `laden_ton_miles_in_transit_{dwt,gas}` (SIGNALS.md #1/#2), `mean_laden_voyage_age_h` (#20), `od_flow_count` (#5), `eu_arrivals` (#4), `us_loadings` (#9). `regime` is segmented per SIGNALS.md §0.5 (never aggregate a model across the 2026-05-30 seam) — tagged by the *leg's* regime (fixed at departure), with a synthetic `'all'` row summing both. `basis='physical'` is the hindsight-clean reconstruction (a leg is live on day `d` iff `departed≤d<arrived`); `'knowable'` (leakage-free point-in-time) is reserved but not yet built. The headline "laden ton-miles in transit" lives here as a `signal_key` value, not a dedicated table.
...
- **`signal.py`** — The signal aggregation layer. Pure aggregators + a thin DB loader; rebuilds `signal_daily` (TRUNCATE + atomic executemany swap, like `port_events`). Consumes `legs.compute_legs()` + raw `port_events` counts. First-build signals: laden ton-miles in transit dwt/gas (#1/#2, US export→EU lane), mean laden-voyage age of open legs (#20), O-D flow matrix (#5), EU arrivals/day (#4), US loadings/day (#9). Ton-mile reconstruction is `basis='physical'` (a leg is live on day `d` iff `departed≤d<arrived`); open legs (no observed arrival) get an *estimated* origin→declared-dest-zone-centroid distance via `geo.haversine_nm`. `--as-of`/`--panel-start` for reproducible rebuilds. Queue-time family (#6/#12) and the leakage-free `basis='knowable'` series are deferred. See `analysis/SIGNALS.md`.
