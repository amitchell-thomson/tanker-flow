...
> **HOST + EARLY-AIS CORRECTION (verified 2026-06-14).** Two findings that
> reshaped the loader:
>
> 1. **NOAA moved hosts.** The legacy `coast.noaa.gov/.../AIS_YYYY_MM_DD.zip` path
>    404s for 2025+. The live data is on an Azure blob —
>    `noaaocm.blob.core.windows.net/ais/csv2/csv{Y}/ais-Y-M-D.csv.zst` (zstd CSV,
>    lowercase `csv2` schema) — which carries **all years 2015→2025** (2025 complete
>    through 12-31; only 2026 not yet up, ~3-month lag). `noaa_ais.py` now points
>    here. So the usable frontier is **2025-12-31**, not 2024 — the ~18-month-lag
>    scare was just the dead URL.
> 2. **2016–2017 was never sparse — it was *untyped*.** A 2016 day is a *full* file
>    (8.4M rows) but ~**46% of fixes carry no `vessel_type`** (vs 1% in 2025), and
>    the registered LNG fleet is **100% untyped** there — so the old `vessel_type
>    80-89` filter dropped every LNG carrier (~11k "tankers"/day, 0 registered LNG).
>    Fix: `read_tankers`/`load_tier2` now select the LNG fleet by **registry IMO,
>    union with typed tankers** (§3.6 identity), regardless of `vessel_type`.
>    Verified: 2016-03-15 went from 0 → **1,646 LNG fixes** in `ais_fixes`. The
>    Sabine-origin story **is** recoverable for the ~735 *known* hulls (re-fetch
>    2016–2017 with `FORCE=1` to override the stale type-only archives). Unknown /
>    scrapped early hulls are still missed until §3.6.1 fills the registry.
...
#### 3.6.1 Berth-sweep registry completion (planned follow-up, post-full-backfill)

The Tier-2 gate (§3.8) only admits LNG hulls **already in `vessel_registry`** into
`ais_fixes`; an LNG carrier we never resolved (an early-era hull, a missed
newbuild) is **not lost** — its tanker fixes sit in the Tier-1 Parquet archive,
which keeps all tankers nationwide with `IMO` + `VesselName`. So once the full
decade archive is on disk we can **retrospectively complete the registry** — the
historical mirror of the live `scripts/discover_berth_tankers.py` /
`run_berth_discovery` path:

1. **Sweep the *archive* (not `ais_fixes` — it's already registry-filtered)** for
   tanker fixes inside a US LNG **berth** polygon (`terminal_zones`,
   `zone_type='berth'`). A tanker physically in a dedicated LNG berth is
   near-certain LNG (the one inference we trust; approach/anchorage presence is
   not). Collect distinct `(imo, vessel_name)`.
2. **Diff vs `vessel_registry`** → the hulls we're missing / haven't flagged
   `is_lng_carrier`.
3. **Resolve the gap.** VF-enrich the gap IMOs (`vesselfinder.py` path; VF bills
   per *returned* record, so hulls it can't resolve cost 0). **Caveat:** VF is a
   *current* database — hulls scrapped before ~2024 may return nothing. Those are
   **not** dropped: fall back to §3.6's admit-from-source-identity (registry row
   from the archive's own `IMO` + `VesselName`, `is_lng_carrier=TRUE`, **fleet-mean
   `gas_capacity_m3`** since the archive carries no dimensions). VF for the living,
   source-identity fallback for the dead.
4. **`make backfill-noaa-reload START=2016-01-01 END=…`** re-scans the Parquet and
   admits the newly-recognised hulls' historical fixes into `ais_fixes` — no
   re-download (`reload_archive`).

Payoff: every recovered hull is a recovered US loading/departure, so this raises
the US capture rate (~77% measured on 2022) — it tightens the corpus exactly where
it is already richest (US supply). Small known floor: hulls broadcasting `imo=0`
in berth are missed by the IMO-keyed diff (MMSI fallback reopens the decade-reuse
problem). One-time, cheap, gated on the full archive being present.
